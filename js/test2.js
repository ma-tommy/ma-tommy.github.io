    ...
    if (
      ...
      if ($img_close.length) {
        var imgCloseId = setInterval(function () {
          // safariの場合に、上部で定義・取得したタイミングでは取れない場合があったので、
          //処理をする直前にDomの情報を精査する
          if ($('#ClaimInfo > h2 ').has("img[alt='ご利用状況']").length == 0) return;  
      ...
      $("td:nth-child(2) span.text-red").css({"vertical-align":"bottom", "display": "inline-block"});
      $("td:nth-child(2) span.text-red a").css("font-size", '11px');

      /* 「お知らせ」と「オリコポイント」の位置を逆に -------stat*/
      $("#base").after($("#footer").prev("div.box1"));
      $("#base").next("div.box1").children().first().attr("style", "margin-top:25px !important; margin-left:-10px !important;");
