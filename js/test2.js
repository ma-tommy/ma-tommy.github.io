...
var id = setInterval(function () {
	...
	if (idNcSpcCountBefore == idNcSpcCountAfter) {
		if ($("img[alt='暮らスマイル情報']").length > 0) {
			console.log("暮らすマイルがミカッタ");
			idNcSpcCountDelay = DELAY_COUNT;
			return;
		} else {
			console.log("暮らすマイルが見つからねぇ");
			idNcSpcCountDelay++;
			return;
		}
	}
	...
	if (
		...
		console.log("bgDotの件数は" + $bg_dot.length);
		if ($bg_dot.length) {
			console.log("bgDotの処理 TRUE");
