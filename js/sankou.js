...
var id = setInterval(function () {
	...
	// NC_spcのIDが全部展開しているかを判定する（念のため速度の都合でカウントが増えてない可能性もあるので０.６秒程待つ)
	if (
	...
	// if (($("img[alt='お知らせ']").length === 0 && $("[id*='NC_spc_']").length >= 4 && $("img[alt='暮らスマイル情報']").length > 0) || ($("img[alt='お知らせ']").length !== 0 && $("[id*='NC_spc_']").length >= 5  && $("img[alt='暮らスマイル情報']").length > 0)) {
	// if ($("[id*='NC_spc_']").length == 4) {
	if (
		idNcSpcCountBefore == idNcSpcCountAfter &&
		idNcSpcCountDelay >= DELAY_COUNT
	) {
